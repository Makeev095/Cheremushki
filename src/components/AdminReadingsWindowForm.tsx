...
export function AdminReadingsWindowForm({
  ...
  async function submit(mode: Mode) {
    ...
    try {
      ...
      setStatus(
        mode === "auto"
          ? "Режим: автоматически по дате (20–24). Страница обновлена."
          : mode === "open"
            ? "Режим: форма всегда открыта (тест). Страница обновлена."
            : "Режим: только уведомление (тест). Страница обновлена.",
      );
      router.refresh();
    } catch {
